...
export default defineEventHandler(async (event) => {
    ...
    const user = await prisma.rapidDamageAssessmentNeedAnalysisReport.create({
        data: {
            status: 'pending',
            summary: validated.data.summary,
            totalDamage: validated.data.totalDamage,
            totalDisplaced: validated.data.totalDisplaced,
            barangay: {
       ...
       return {
      success: true,
      message: 'Rdana created successfully!',
      barangay,
    }
})